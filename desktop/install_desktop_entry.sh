...
# ============================================================================
# Install the home_inventory launcher icon and .desktop entry.
#
# The desktop app is a Flutter *web* build shown in a Chrome app window (the
# Linux embedder manages only ~20fps at 4K — see the README), so the launcher
# starts the wrapper, which serves the build and opens the browser.
#
...
readonly ICON_NAME="home-inventory"
# Chrome sets WM_CLASS from --class, which the wrapper passes explicitly
# (bin/home_inventory_desktop.dart). Without it the window would inherit the
# browser's own class (e.g. "Google-chrome") and the taskbar would show a
# browser icon for the app.
readonly WM_CLASS="home-inventory"
...
LAUNCH_CMD="/usr/bin/home-inventory"
...
Type=Application
Name=Home Inventory
Comment=Offline-first, CRDT-synced household inventory
Exec=$LAUNCH_CMD
Icon=$ICON_NAME
Terminal=false
