...
class SyncSettings {
  ...
  /// GitHub repository name data syncs to.
  final String repo;
  ...
  /// Loads settings, defaulting to the shared `kuhyx/syncs` repo and the
  /// baked-in [defaultClientId], so first run — and any reinstall — needs
  /// nothing but a single "Connect GitHub" tap.
  ///
  /// Unlike the older apps there is no legacy per-app repo name to migrate
  /// from here: this app has only ever used the shared `syncs` repo.
  static Future<SyncSettings> load() async {
    final prefs = await SharedPreferences.getInstance();
    return SyncSettings(
      owner: prefs.getString(_kOwner) ?? 'kuhyx',
      repo: prefs.getString(_kRepo) ?? 'syncs',
