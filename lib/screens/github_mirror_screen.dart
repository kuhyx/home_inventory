...
import 'package:home_inventory/sync/github_mirror_actions.dart';
...
import 'package:home_inventory/ui/github_mirror_form.dart';
...
class _GitHubMirrorScreenState extends State<GitHubMirrorScreen> {
  ...
  SyncSettings _current() {
    ...
    return base.copyWith(owner: _owner.text.trim(), repo: _repo.text.trim());
  }
  ...
  Future<void> _testConnection() => _guard(() async {
    ...
    return await testGitHubConnection(settings, httpClient: widget.httpClient);
  });
  ...
  @override
  Widget build(BuildContext context) => GitHubMirrorForm(
    ...
    busy: _busy,
    status: _status,
    nodeId: widget.repository.nodeId,
    onConnect: _connect,
