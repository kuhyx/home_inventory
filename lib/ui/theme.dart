...
ThemeData _buildTheme(ColorScheme colorScheme, AppStatusColors statusColors) {
  return ThemeData(
    ...
    textTheme: const TextTheme(
      ...
      // (empty-state messages, item names, quantities) with no explicit
      // style, so this must also sit at the 16px reading floor rather than
      // M3's 14px default.
...
/// every stock indicator should read these rather than an ad hoc
/// `Colors.green`/`Colors.amber` literal.
///
/// Warning carries real meaning here: it is the "running low" state, i.e. an
/// item at or under its `lowStockAt` threshold. `error` (danger) is reserved
/// for "out" — a quantity of zero.
@immutable
class AppStatusColors extends ThemeExtension<AppStatusColors> {
  ...
  /// The shared dark-theme instance — identical values; the unified
  ...
  /// Positive/in-stock status (an item comfortably above its threshold).
  final Color success;

  /// Caution status — an item at or below its `lowStockAt` threshold.
  final Color warning;
